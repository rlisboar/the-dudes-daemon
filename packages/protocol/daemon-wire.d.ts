/**
 * Daemon ↔ Orchestrator wire protocol — CONTRATO (T-423/R3).
 *
 * Movido de daemon/src/protocol.ts: o pacote protocol é a fonte única dos
 * tipos e dos schemas zod (`daemon-wire.js`). daemon/src/protocol.ts apenas
 * reexporta daqui + mantém os 2 helpers puros.
 *
 * Tipos compartilhados com o wire web↔server vêm de `./wire` (fecha M34:
 * AgentInfo/AgentUsage/ImageAttachment/Skill e MCP deixam de ser redefinidos).
 */
import type {
  AgentInfo,
  AgentRepo as AgentRepoSpec,
  AgentRuntimeState,
  AgentUsage,
  CliRunner,
  EffortLevel,
...
export type { GitFileStatus, RunnerModelCatalog, DiscoveredRunnerModel } from "./wire";

/** Repo base do workspace (workspace:set) — espelho do que o daemon clona. */
...
/** type → schema zod (T-423). Chave é o `type` exato do FromDaemon. */
export declare const daemonWireSchemas: Record<string, unknown>;

/** type → schema zod das mensagens server → daemon (FromOrch). */
export declare const fromOrchSchemas: Record<string, unknown>;

/** Valida mensagem recebida do daemon, fail-closed. */
export declare function validateDaemonMessage(
...
export declare function cipherWirePrefix(text: string): "e2e:v2" | "e2e:v1" | "e2e" | "none";
...
export declare function assembleAgentSendParts(
  parts: AgentSendPart[],
