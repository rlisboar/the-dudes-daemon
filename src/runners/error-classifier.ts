...
// T-414/A14: sem "404 not found" genérico — um GET 404 de tool apagava sessionId.
// Textos reais dos CLIs (claude/qwen/crush/grok) continuam nos ramos abaixo.
const MISSING_SESSION_RE = /no conversation found with session id|session not found|couldn't (?:start|load|resume) session|no such session|no saved session found/i;
