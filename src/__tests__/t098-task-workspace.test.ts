...
test("T-098 create: worktree+branch a partir de main", async () => {
  const repo = fixtureRepo();
  const r = await createTaskWorktree({ workspaceRoot: repo, taskId: "task_1", agentId: "ag_be" });
  ...
  const listed = await listLocalWorktrees(repo);
  assert.ok(listed.some((w) => w.path === r.path && w.branch === "ag_be/task_1"));
});

test("T-098 colisão: branch existe → erro, nada sobrescrito", async () => {
  const repo = fixtureRepo();
  const a = await createTaskWorktree({ workspaceRoot: repo, taskId: "task_1", agentId: "ag_be" });
  assert.equal(a.ok, true);
  const readme = a.ok ? path.join(a.path, "README") : "";
  writeFileSync(readme, "keep-me\n");
  const b = await createTaskWorktree({ workspaceRoot: repo, taskId: "task_1", agentId: "ag_be" });
...
test("T-098 remove: mergeada (sem commits extra) remove worktree+branch", async () => {
  const repo = fixtureRepo();
  const a = await createTaskWorktree({ workspaceRoot: repo, taskId: "task_2", agentId: "ag_qa" });
  assert.equal(a.ok, true);
  if (!a.ok) return;
  const rm = await removeTaskWorktree({ workspaceRoot: repo, path: a.path, branch: a.branch });
...
test("T-098 remove: NÃO-mergeada recusa e lista commits; force remove", async () => {
  const repo = fixtureRepo();
  const a = await createTaskWorktree({ workspaceRoot: repo, taskId: "task_3", agentId: "ag_fe" });
  assert.equal(a.ok, true);
  if (!a.ok) return;
  writeFileSync(path.join(a.path, "feat.txt"), "x\n");
  git(a.path, ["add", "feat.txt"]);
  git(a.path, ["commit", "-m", "feat pendente"]);
  const deny = await removeTaskWorktree({ workspaceRoot: repo, path: a.path, branch: a.branch });
  ...
  const forced = await removeTaskWorktree({ workspaceRoot: repo, path: a.path, branch: a.branch, force: true });
  assert.equal(forced.ok, true, forced.ok ? "" : forced.error);
  assert.equal(existsSync(a.path), false);
});

test("T-098 list devolve worktrees locais", async () => {
  const repo = fixtureRepo();
  await createTaskWorktree({ workspaceRoot: repo, taskId: "task_l", agentId: "ag_x" });
  const list = await listLocalWorktrees(repo);
