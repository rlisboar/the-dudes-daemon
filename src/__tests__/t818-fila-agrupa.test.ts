...
test("T-818: fila no teto agrupa na última mensagem, na ordem, sem perder nada", () => {
  ...
  const principal = { isAgentOwner: true } as const;
  assert.equal(s.enqueueOrCoalesce({ content: "m1", principal }, 2, 1_000), "queued");
  assert.equal(s.enqueueOrCoalesce({ content: "m2", principal }, 2, 1_000), "queued");
  assert.equal(s.enqueueOrCoalesce({ content: "m3", principal, images: [{ mediaType: "image/png", data: "x" }] as never }, 2, 1_000), "coalesced");
  assert.equal(s.enqueueOrCoalesce({ content: "m4", principal }, 2, 1_000), "coalesced");
...
test("T-818: mensagem de usuário não entra na sintética (hang-recover fica fora do dreno)", () => {
  const s = new PerMessageSessionState();
  const principal = { isAgentOwner: true } as const;
  s.enqueue({ content: "u1", principal }, 5);
  s.enqueue({ content: "nudge", synthetic: "hang-recover" }, 5);
  assert.equal(s.enqueueOrCoalesce({ content: "u2", principal }, 2, 1_000), "coalesced");
...
test("T-818: passou do teto de BYTES → descarta e não mexe no que já estava na fila", () => {
  const s = new PerMessageSessionState();
  const principal = { isAgentOwner: true } as const;
  s.enqueue({ content: "a".repeat(60), principal }, 1);
  assert.equal(s.enqueueOrCoalesce({ content: "b".repeat(60), principal }, 1, 100), "dropped");
  ...
  utf8.enqueue({ content: "x".repeat(20), principal }, 1);
  assert.equal(utf8.enqueueOrCoalesce({ content: "é".repeat(30), principal }, 1, 20 + sep + 60), "coalesced", "cabe em bytes com o separador");
  const utf8b = new PerMessageSessionState();
  utf8b.enqueue({ content: "x".repeat(20), principal }, 1);
  assert.equal(utf8b.enqueueOrCoalesce({ content: "é".repeat(30), principal }, 1, 20 + sep + 59), "dropped", "59 bytes não cabem 30 é (60 bytes)");
...
function runnerComTurnoEmCurso(cliRunner: string) {
  ...
  const ownerPrincipal = { isAgentOwner: true } as const;
  const ownerSend = (content: string) => runner.pushUserMessage(content, undefined, undefined, undefined, ownerPrincipal);
  a.drainOcQueue = () => {}; // turno longo em curso: nada sai da fila
  a.messageSession.busy = true;
  return { runner, ownerSend, a, logs, avisos };
}

test("T-818: runner per-message com 25 mensagens num turno longo não descarta nenhuma e avisa UMA vez", () => {
  const { runner, ownerSend, a, logs, avisos } = runnerComTurnoEmCurso("grok-custom");
  for (let i = 1; i <= 25; i++) ownerSend(`notificação ${i}`);
...
test("T-818: o aviso volta a valer só depois de a fila cair abaixo da metade", () => {
  const { ownerSend, a, avisos } = runnerComTurnoEmCurso("grok");
  for (let i = 0; i < 21; i++) ownerSend(`r1-${i}`);
  ...
  for (let i = 0; i < 16; i++) ownerSend(`r2-${i}`);
...
test("T-818: flood acima do teto de 64 KiB é descartado COM aviso no chat — um aviso por rajada", () => {
  const { ownerSend, logs, avisos } = runnerComTurnoEmCurso("opencode");
  const grande = "x".repeat(40_000);
  for (let i = 0; i < 20; i++) ownerSend(`q${i}`);
  for (let i = 0; i < 4; i++) ownerSend(grande);
...
test("T-818: descarte do buffer de restart do claude e da fila do dsh também é declarado", () => {
  ...
  for (let i = 0; i < 21; i++) claude.ownerSend(`c${i}`);
  ...
  for (let i = 0; i < 60; i++) dsh.ownerSend(`d${i}`);
