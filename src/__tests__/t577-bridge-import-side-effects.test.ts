...
function esperaLinha(child: ChildProcess, re: RegExp, ms: number): Promise<string> {
  return new Promise((resolve, reject) => {
    let buf = "";
    const t = setTimeout(() => reject(new Error(`stdout não bateu ${re} em ${ms}ms: ${JSON.stringify(buf)}`)), ms);
...
test("T-577: import do bridge com stdin aberto sai sozinho (não pendura o runner)", async () => {
  ...
  try {
    const code = await esperaSaida(child, 15_000);
...
test("T-577: bridge como entrypoint sem AGENT_ID continua falhando alto (exit 1)", async () => {
...
test("T-577: entrypoint pelo bin do tsx (dev) também age como processo", { skip: !existsSync(TSX_BIN) }, async () => {
...
test("T-577: bridge como entrypoint conecta o stdio (initialize responde)", async () => {
  ...
  try {
    ...
    const out = await esperaLinha(child, /"id":1/, 15_000);
